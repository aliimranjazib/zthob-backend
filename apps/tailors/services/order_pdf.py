...
def _ltr_value_html(value, lang='en'):
    """Keep Western dates/times left-to-right inside RTL ReportLab paragraphs."""
    if value is None or value == '':
        return '—'
    text = str(value)
    if not _is_rtl(lang):
        return _safe_text(text)
    return _safe_text(f'{_LRM}{text}{_LRM}')
...
def _kv_table(rows, col_widths=None, lang='en'):
    ...
    for row in rows:
        ...
        if not val:
            val_html = '—'
        elif skip_trans == 'datetime':
            val_html = _ltr_value_html(val, lang)
        elif skip_trans:
        ...
        if rider_grid:
            ...
            if order.measurement_taken_at:
                rider_block.append(Paragraph(
                    f'<i>{_safe_text(_t("Measured at:", lang))} {_ltr_value_html(_fmt_datetime(order.measurement_taken_at), lang)}</i>',
                    s['small'],
                ))
...
def _order_info_rows(order, lang):
    ...
    if order.estimated_delivery_date:
        rows.append(('Est. Delivery', _fmt_date(order.estimated_delivery_date), 'datetime'))
    if order.actual_delivery_date:
        rows.append(('Actual Delivery', _fmt_date(order.actual_delivery_date), 'datetime'))
    if order.appointment_date:
        appt = _fmt_date(order.appointment_date)
        if order.appointment_time:
            appt += f' at {order.appointment_time.strftime("%I:%M %p")}'
        rows.append(('Appointment', appt, 'datetime'))
    if order.stitching_completion_date:
        rows.append(('Stitching Done', _fmt_date(order.stitching_completion_date), 'datetime'))
    return rows
...
def _build_header_section(order, page_w, s, lang):
    ...
    status_data = [[
        ...
        Paragraph(
            f'{_safe_text(_placed_lbl)} <b>{_ltr_value_html(_fmt_datetime(order.created_at), lang)}</b>',
...
def _build_status_history_section(order, page_w, s, lang):
    ...
    for h in history:
        ...
        hist_row = [
            Paragraph(_ltr_value_html(_fmt_datetime(h.created_at), lang), s['small']),
...
def _build_comments_and_footer(order, page_w, s, lang):
    ...
    if is_rtl:
        ...
        footer_text = (
            f'{_order_label} {order.order_number}  ·  '
            f'{_LRM}{generated_time}{_LRM}  ·  {_gen_label}'
        )
    else:
